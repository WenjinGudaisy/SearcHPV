...
window = config_dic[other_para]['window']
...
if listSites != ['']:  
    for site in listSites:
        ...
        try:
            ...
            print("Contig cound not mapping to HPV",sample,site)
            continue
...
with open(outputPath + 'HPVfusionPointContig.txt','w') as output: 
    output.write('sampleName\tcontigName\tgenomeInsertionChromosome\tgenomeInsertionPoint\thpvInsertionPoint\tSiteConfidence\tContigConfidence\n')
...
#         if key != 'Sample_81279.3.189612850':
        sampleName = key.split('.')[0]
        chro = key.split('.')[1]
        if 'GL' not in chro:
            genomeInsertion = key.split('.')[2]
            ...
            for eachContig in value:
                ...
                output.write(f'{sampleName}\t{contigName}\t{chro}\t{genomeInsertion}\t {hpvInsertion}\t{siteConfOutput}\t{contigConfidence}\n')
            output.write('\n')
